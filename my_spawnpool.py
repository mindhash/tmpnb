...
class SpawnPool(): 
  ...
  def _wait_for_server(self, ip, port, path, timeout=10, wait_time=0.2):
      ...
      while loop.time() - tic < timeout:
          try:
              ...
              app_log.debug("Booting server at [%s], getting HTTP status [%s]", path, code)
              yield gen.Task(loop.add_timeout, loop.time() + wait_time)
          else:
              break
  ...
  def heartbeat(self):
    ...
    try:
      ...
      app_log.info("Heartbeat begun. Measuring current state.")
  ...
  def _launch_container(self, user=None, enpool=True):
    ...
    app_log.debug("Created notebook server [%s] for path [%s] at [%s:%s]", container_name, path, 2, host_port)
    ...
    app_log.info('proxy point %s', proxy_endpoint)
